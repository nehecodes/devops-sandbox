...
while true; do
  ...
  # Fixed: use find instead of glob with 2>/dev/null redirect
  while IFS= read -r STATE_FILE; do
    [[ -f "$STATE_FILE" ]] || continue

    ENV_ID=$(basename "$STATE_FILE" .json)

    CREATED_TS=$(python3 -c "import json; d=json.load(open('$STATE_FILE')); print(d.get('created_ts', 0))" 2>/dev/null || echo 0)
    ...
    if [[ $NOW -gt $EXPIRES_AT ]]; then
      log "TTL expired for $ENV_ID (${REMAINING#-}s ago, status=$STATUS) — destroying"
  ...
  done < <(find "$ENVS_DIR" -maxdepth 1 -name '*.json' 2>/dev/null)
